...
app.get('/search', async (req, res) => {
    ...
    const ingredientChecks = Array.from({ length: 29 }, (_, i) => `ingredient${i + 1} IN (${placeholders})`);
    const query = `
        SELECT *
        FROM ingredientData
        WHERE ${ingredientChecks.join(' OR ')}
    `;

    const placeholders2 = not_ingredients.map(() => '?').join(', ');
    const ingredientChecks2 = Array.from({ length: 29 }, (_, i) => `ingredient${i + 1} IN (${placeholders2})`);
    const query2 = `
        SELECT *
        FROM ingredientData
        WHERE ${ingredientChecks2.join(' OR ')}
    `;
    ...
    conn.execute(query, values, async (err, results) => {
        ...
        for(const result in results) {
            input_recipeID.push(results[result].ID);
        }
        
        const recipeId = [];
        const recipes = [];

        conn.execute(query2, values2, async (err, results) => {
            if (err) {
                console.error('쿼리 실행 중 오류 발생:', err);
                return;
            }
            for(const result in results) {
                input_recipeID2.push(results[result].ID);
            }
